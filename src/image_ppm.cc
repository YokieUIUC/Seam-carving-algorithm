...
void ImagePPM::RemoveHorizontalSeam(const int* path) {
  auto** newmap = new Pixel*[height_ - 1];
  for (int row = 0; row < height_ - 1; row++) {
    newmap[row] = new Pixel[width_];
  }
  for (int col = 0; col < width_; col++) {
    for (int row = 0; row < height_ - 1; row++) {
      if (row >= path[col]) {
        newmap[row][col] = pixels_[row + 1][col];
      } else {
        newmap[row][col] = pixels_[row][col];
      }
    }
  }
  for (int row = 0; row < height_; row++) {
...
void ImagePPM::RemoveVerticalSeam(const int* path) {
  auto** newmap = new Pixel*[height_];
  for (int row = 0; row < height_; row++) {
    newmap[row] = new Pixel[width_ - 1];
  }
  for (int row = 0; row < height_; row++) {
    for (int col = 0; col < width_ - 1; col++) {
      if (col < path[row]) {
        ...
        newmap[row][col] = pixels_[row][col + 1];
      }
    }
  }
  for (int row = 0; row < height_; row++) {
...
std::ostream& operator<<(std::ostream& os, const ImagePPM& image) {
  ...
  for (int i = 0; i < image.height_; i++) {
    for (int j = 0; j < image.width_ - 1; j++) {
      os << image.pixels_[i][j].GetRed() << '\n'
         << image.pixels_[i][j].GetGreen() << '\n'
         << image.pixels_[i][j].GetBlue() << '\n';
    }
    os << image.pixels_[i][image.width_ - 1].GetRed() << '\n'
       << image.pixels_[i][image.width_ - 1].GetGreen() << '\n'
       << image.pixels_[i][image.width_ - 1].GetBlue() << '\n';
  }
  return os;
}